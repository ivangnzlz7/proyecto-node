...
export const updateItemQuantity = async (carrito_id, cantidad) => {
  ...
  try {
    ...
    const [item] = await pool.query('SELECT producto_id FROM carrito WHERE id = ?', [carrito_id]);
    if (item.length === 0) {
    ...
    await pool.query('UPDATE carrito SET cantidad = ? WHERE id = ?', [cantidad, carrito_id]);
...
export const removeItem = async (carrito_id) => {
  try {
...
export const totalCarrito = async () => {
  try {
    ...
    const productos = await getCartItems();
